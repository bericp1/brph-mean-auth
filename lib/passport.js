      ...
      function(email, password, done) {
        User.findOne({ 'local.email' :  email }, function(err, user) {
          ...
          if (!user || !utils.validPassword(user, password))
            return done(null, false, {error:errors.login.bad, fields: ['email', 'password']});
      ...
      function(token, done){
        User.findOne({ 'local.token.value': token }, function(err, user){
          if(err)
            return done(err);
          if(!user)
            return done(null, false, {error:errors.token.invalid, fields: ['token']});
          if(!utils.validToken(user))
            return done(null, false, {error:errors.token.expired, fields: ['token']});
